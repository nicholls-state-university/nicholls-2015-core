
<!-- #nicholls-footer-wrapper - START -->
<div id="nicholls-footer-wrapper" class="nicholls-footer-wrapper-">
	<footer id="nicholls-site-footer" class="nicholls-site-footer-" role="contentinfo">

	<div class="nicholls-footer-column nicholls-footer-col-info">

		<div class="nicholls-la-state-map">
		<a class="link-nicholls-about-directions" href="//www.nicholls.edu">Nicholls State University Directions</a>
		</div>

		<div class="nicholls-info-footer">

			<div class="nicholls-logo-wordmark">
			<a class="link-nicholls-homepage" href="//www.nicholls.edu">Nicholls State University</a>
			</div>

			<div class="nicholls-phone-number">1-877-NICHOLLS</div>

			<div class="nicholls-footer-info-navigation">
			<ul>
				<li><a href="//www.nicholls.edu/contact/">Contact Us</a></li>
				<li><a href="//www.nicholls.edu/about/campus-map/">Maps</a></li>
				<li><a href="//www.nicholls.edu/about/directions/">Directions</a></li>
			</ul>
			</div>

			<div class="nicholls-address">906 East 1st St. Thibodaux, Louisiana 70301</div>

		</div>

	</div>

	<div class="nicholls-footer-column nicholls-footer-col-social-media">

		<div class="nicholls-social-media">
		<ul>
			<li class="footer-icon footer-icon-facebook"><a href="//www.facebook.com/Nicholls-State-University/">Nicholls Facebook</a></li>
			<li class="footer-icon footer-icon-instagram"><a href="//www.instagram.com/nichollsstate/">Nicholls Instagram</a></li>
			<li class="footer-icon footer-icon-twitter"><a href="//twitter.com/nichollsstate/">Nicholls Twitter</a></li>
			<li class="footer-icon footer-icon-linkedin"><a href="//www.linkedin.com/edu/nicholls-state-university-18432">Nicholls LinkedIn</a></li>
			<li class="footer-icon footer-icon-youtube"><a href="//www.youtube.com/nichweb">Nicholls YouTube</a></li>
		</ul>
		</div>

	</div>

	<div class="nicholls-footer-column nicholls-footer-col-nav">

		<div class="nicholls-footer-menu">

			<div class="nicholls-footer-menu-column">
				<ul>
					<li><a class="icon-inline-small-mail" href="//www.nicholls.edu/student-email/">Student Email</a></li>
					<li><a class="icon-inline-small-mail" href="//mail.google.com/a/nicholls.edu">Faculty &amp; Staff Email</a></li>
					<li><a class="icon-inline-small-document" href="//banner.nicholls.edu/">Banner</a></li>
					<li><a class="icon-inline-small-document" href="http://moodle.nicholls.edu/moodle/">Moodle</a></li>
					<li><a class="icon-inline-small-document" href="https://nicholls.gradesfirst.com/">Grades First</a></li>
					<li><a class="icon-inline-small-document" href="//www.nicholls.edu/library/">Library</a></li>
				</ul>
			</div>

			<div class="nicholls-footer-menu-column">
				<ul>
					<li><a class="icon-inline-small-question" href="//www.nicholls.edu/site-map/">Website Directory</a></li>
					<li><a class="icon-inline-small-question" href="//www.nicholls.edu/directory/">Faculty &amp; Staff Directory</a></li>
					<li><a class="icon-inline-small-ada" href="//www.nicholls.edu/ada/">ADA Information</a></li>
					<li><a class="icon-inline-small-emergency" href="http://emergency.nicholls.edu/">Emergency Preparedness</a></li>
					<li><a class="icon-inline-small-jobs" href="//www.nicholls.edu/human-resources/">Employment</a></li>
					<li><a class="icon-inline-small-give" href="http://www.nichollsfoundation.org">Give to Nicholls</a></li>
				</ul>
			</div>

		</div>

	</div>

	<div class="nicholls-info-copyright">All content and images &copy;<?php echo date('Y'); ?> Nicholls State University - Website <a href="//www.nicholls.edu/disclaimer/">Policy &amp; Disclaimer</a> - Member of the <a href="//ulsystem.edu/">University of Louisiana System</a></div>

	</footer>
</div>
<!-- #nicholls-footer-wrapper - END -->
